...
在JDK 5之前[Java](http://lib.csdn.net/base/java)语言是靠synchronized关键字保证同步的，这会导致有锁
...
类似于 CAS 的指令允许[算法](http://lib.csdn.net/base/datastructure)执行读-修改-写操作，而无需害怕其他线程同时 修改变量，因为如果其他线程修改变量，那么 CAS 会检测它（并失败），算法 可以对该操作重新计算。
...
关于ABA问题参考文档: http://blog.hesey[.NET](http://lib.csdn.net/base/dotnet)/2011/09/resolve-aba-by-atomicstampedreference.html
...
AQS，非阻塞[数据结构](http://lib.csdn.net/base/datastructure)和原子变量类（java.util.concurrent.atomic包中的类），这些concurrent包中的基础类都是使用这种模式来实现的，而concurrent包中的高层类又是依赖于这些基础类来实现的。从整体来看，concurrent包的实现示意图如下：

![img](image-201708301525/b7b2472f-6b93-3f85-9e44-29a9ff774c8e.png)
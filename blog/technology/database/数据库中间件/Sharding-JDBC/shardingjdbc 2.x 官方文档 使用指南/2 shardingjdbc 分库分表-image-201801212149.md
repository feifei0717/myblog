...
[![策略类图](image-201801212149/StrategyClass.900.png)](http://shardingjdbc.io/1.x/docs/img/StrategyClass.900.png) Sharding-JDBC认为对于分片策略存有两种维度 - 数据源分片策略DatabaseShardingStrategy：数据被分配的目标数据源 - 表分片策略TableShardingStrategy：数据被分配的目标表，该目标表存在与该数据的目标数据源内。故表分片策略是依赖与数据源分片策略的结果的 这里注意的是两种策略的API完全相同，以下针对策略API的讲解将适用于这两种策略
...
[![算法](image-201801212149/AlgorithmClass.900.png)](http://shardingjdbc.io/1.x/docs/img/AlgorithmClass.900.png)
...
> 如果希望不依赖于表中的列传入分片键值，参考：[强制路由](http://shardingjdbc.io/1.x/docs/02-guide/hint-sharding-value)



http://shardingjdbc.io/1.x/docs/02-guide/sharding/